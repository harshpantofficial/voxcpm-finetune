...
def train(
    ...
    # Checkpoint sync with the HF Hub (resume across ephemeral/preemptible nodes)
    hub_repo_id: str = "",
    hub_token: str = "",
):
    _ = config_path
    ...
    if accelerator.rank == 0:
        pull_checkpoint_from_hub(save_dir, hub_repo_id, hub_token)
    accelerator.barrier()
    ...
    def _signal_handler(
        ...
        _hf_id=hf_model_id,
        _dist=distribute,
        _hub_repo=hub_repo_id,
        _hub_token=hub_token,
        _resume=resume,
        ...
        if _rank == 0:
            ...
            try:
                ...
                push_checkpoint_to_hub(_save_dir, _hub_repo, _hub_token, cur_step)
    ...
    with tracker.live():
        for step in range(start_step, num_iters):
            ...
            if (step % save_interval == 0 or step == num_iters - 1) and accelerator.rank == 0:
                ...
                push_checkpoint_to_hub(save_dir, hub_repo_id, hub_token, step)

    if accelerator.rank == 0:
        save_checkpoint(model, optimizer, scheduler, save_dir, num_iters, pretrained_path, hf_model_id, distribute)
        push_checkpoint_to_hub(save_dir, hub_repo_id, hub_token, num_iters)
    if writer:
        writer.close()
...
def pull_checkpoint_from_hub(save_dir: Path, repo_id: str, token: str):
    """
    Downloads an existing checkpoint repo into save_dir so the normal local
    resume path (save_dir/latest) picks it up. No-op if repo_id is empty, a
    local checkpoint already exists, or the repo doesn't exist yet (first run).
    """
    if not repo_id or (save_dir / "latest").exists():
        return
    from huggingface_hub import snapshot_download

    try:
        snapshot_download(repo_id=repo_id, token=token or None, local_dir=str(save_dir))
        print(f"Pulled existing checkpoint from {repo_id}", file=sys.stderr)
    except Exception as e:
        print(f"No existing checkpoint pulled from {repo_id} ({e}); starting fresh", file=sys.stderr)


def push_checkpoint_to_hub(save_dir: Path, repo_id: str, token: str, step: int):
    """Uploads the full checkpoints directory (all step_* folders + latest/) to a
    public HF Hub model repo, creating it if needed. No-op if repo_id is empty."""
    if not repo_id:
        return
    ...
    try:
        ...
        print(f"Pushed checkpoint (step {step}) to {repo_id}", file=sys.stderr)
    except Exception as e:
        print(f"Warning: failed to push checkpoint to {repo_id}: {e}", file=sys.stderr)
